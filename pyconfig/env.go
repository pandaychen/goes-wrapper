...
// 自动将容器环境的environment变量转换为struct结构
...
func FillConfig(v interface{}) error {
	return env.Fill(v)
}

func (e *EnvironmentConfig) Fill(v interface{}) error {
...
func fill(pf string, ind reflect.Value) error {
	for i := 0; i < ind.NumField(); i++ {
		...
		p := combine(pf, strings.ToUpper((name)), s, exist)
